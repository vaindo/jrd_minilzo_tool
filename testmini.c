...
int jrd_minilzo_compress_file(int fd_in, int fd_out)
{
    ...
    if (0 == wrkmem)
    {
        printf("[minilzo]wrkmem malloc fail.");
        return -1;
    }
    ...
    {
        printf("[minilzo] No memory for cmprs_buffer or buffer! cmprs_buffer \n");
        ret = -1;
        goto _exit;
    }
    ...
    for (i = 0; i < file_size; i += COMPRESS_BUFFER_SIZE)
    {
        ...
        ret = lzo1x_1_compress(buffer, block_size, cmprs_buffer, (lzo_uint *)(&cmprs_size), wrkmem);
        if (ret != LZO_E_OK)
        {
            printf("[minilzo]lzo1x_1_compress error : ret:  %d. ", ret);
            ret = -1;
            goto _exit;
        }
...
int jrd_minilzo_decompress_file(int fd_in, int fd_out)
{
    ...
    for (i = 0; i < file_size; i += BLOCK_HEADER_SIZE + block_size)
    {
        ...
        ret = lzo1x_decompress(buffer, block_size, dcmprs_buffer, (lzo_uint *)&dcmprs_size, NULL);
        if (ret != LZO_E_OK)
        {
            ret = -1;
            goto _dcmprs_exit;
...
int main(int argc, char ** argv)
{
    ...
    */
    ...
    if(memcmp("-c", argv[1], strlen(argv[1])) == 0)
    {
        ...
        if(jrd_minilzo_compress_file(fd_in, fd_out) < 0)
        {
    ...
    else if(memcmp("-d", argv[1], strlen(argv[1])) == 0)
    {
        ...
        if(jrd_minilzo_decompress_file(fd_in, fd_out) < 0)
        {
